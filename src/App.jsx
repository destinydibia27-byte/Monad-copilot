  ...
  return (
    <div style={{ minHeight: "100vh", background: T.bg, color: T.text, position: "relative" }}>
      ...
      <div style={{
        ...
        background: `radial-gradient(ellipse 55% 35% at 75% 8%, ${T.purple}0D 0%, transparent 65%),
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 1140, margin: "0 auto", padding: isMobile ? "0 16px" : "0 24px" }}>
        <header style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 14, width: isMobile ? "100%" : "auto" }}>
            ...
            <div style={{ flex: "1 1 0", minWidth: 0 }}>
              <h1 style={{ fontFamily: "'Syne',sans-serif", fontSize: isMobile ? 18 : 21, fontWeight: 800, letterSpacing: "-0.04em", lineHeight: 1 }}>
                <span style={{ color: "#fff" }}>Monad CT </span>
                <span style={{ color: "#6D5FD8" }}>Co-Pilot</span>
              </h1>
              <div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 9.5, color: T.textDim, letterSpacing: "0.06em", marginTop: 5, display: "flex", alignItems: "center", gap: 6 }}>
                {isMobile ? (
                  <>
                    <span>ECOSYSTEM</span>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, width: isMobile ? "100%" : "auto" }}>
            {user && (
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <button
                  ...
                  style={{ background: "none", border: `1px solid ${T.border2}`, color: T.textDim, padding: "5px 10px", borderRadius: 6, fontFamily: "'IBM Plex Mono',monospace", fontSize: 9.5, letterSpacing: "0.06em", cursor: "pointer", transition: "color 0.15s, border-color 0.15s" }}
            ...
            {generating && (
              <div style={{ display: "flex", alignItems: "center", gap: 7, fontFamily: "'Inter',sans-serif", fontSize: 12, fontWeight: 450, color: T.purpleL, letterSpacing: "-0.01em" }}>
                <Dot color={T.purple} pulse />Generating
              </div>
            )}
            <button
              onClick={() => setShowGenPanel(true)}
              style={{ background: T.purple, color: "#fff", border: "none", padding: isMobile ? "11px 0" : "10px 18px", width: isMobile ? "100%" : "auto", borderRadius: 8, fontFamily: "'Inter',sans-serif", fontSize: 13, fontWeight: 500, letterSpacing: "-0.01em", cursor: "pointer", transition: "opacity 0.15s", boxShadow: `0 4px 16px ${T.purple}35` }}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(140px,1fr))", gap: 10, marginBottom: 28 }}>
          {[
            ...
            <StatCard key={label} label={label} val={val} color={color} sub={sub}
              active={filter ? filterStatus === filter : false}
              onClick={filter ? () => { setFilterStatus(prev => prev === filter ? "All" : filter); setActiveTab("drafts"); } : undefined}
            />
          ))}
        </div>
        ...
        <div style={{ display: "flex", gap: 0, marginBottom: 20, borderBottom: `1px solid ${T.border}` }}>
          {[
            ...
            <button key={tab.key} onClick={() => setActiveTab(tab.key)}
              style={{ background: "none", border: "none", cursor: "pointer", fontFamily: "'Inter',sans-serif", fontSize: 13, fontWeight: activeTab === tab.key ? 500 : 400, letterSpacing: "-0.01em", padding: "10px 20px 11px", marginBottom: -1, color: activeTab === tab.key ? "#fff" : T.textDim, borderBottom: activeTab === tab.key ? `2px solid ${T.purple}` : "2px solid transparent", transition: "color 0.15s, border-color 0.15s", display: "flex", alignItems: "center", gap: 8 }}
            ...
            >
              {tab.label}
              <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 9, fontVariantNumeric: "tabular-nums", padding: "2px 6px", borderRadius: 3, background: activeTab === tab.key ? `${T.purple}20` : T.border, color: activeTab === tab.key ? T.purple : T.textDim }}>{tab.count}</span>
            </button>
          ))}
        </div>
        {activeTab === "drafts" && (
          <div>
            {showGenPanel && <GenPanel selectedCount={selectedIds.length} onGenerate={handleGenerate} onCancel={() => setShowGenPanel(false)} generating={generating} />}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {filteredDrafts.length === 0 ? (
                ...
                <DraftCard key={d.id} draft={d} updates={allUpdates}
        ...
        {activeTab === "updates" && (
          <div>
            {showGenPanel && <GenPanel selectedCount={selectedIds.length} onGenerate={handleGenerate} onCancel={() => { setShowGenPanel(false); setSelectedIds([]); }} generating={generating} />}
            <div style={{ display: "flex", gap: 12, marginBottom: 16, flexWrap: "wrap", alignItems: "center" }}>
              {SOURCES.map(s => (
                <div key={s} style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 12px", background: T.card, border: `1px solid ${T.border}`, borderRadius: 6 }}>
                  <SrcChip source={s} />
                  <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 9, color: T.border2 }}>·</span>
                  <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 9.5, color: T.textDim, letterSpacing: "0.01em" }}>{allUpdates.filter(u => u.source === s).length} updates</span>
                </div>
              ))}
              {selectedIds.length > 0 && (
                <button onClick={() => setShowGenPanel(true)}
                  style={{ marginLeft: isMobile ? 0 : "auto", width: isMobile ? "100%" : "auto", background: T.purple, border: "none", color: "#fff", borderRadius: 7, padding: "9px 16px", fontFamily: "'Inter',sans-serif", fontSize: 13, fontWeight: 500, cursor: "pointer", letterSpacing: "-0.01em", boxShadow: `0 4px 16px ${T.purple}30`, transition: "opacity 0.15s" }}
                  onMouseEnter={e => e.currentTarget.style.opacity = "0.85"}
                  onMouseLeave={e => e.currentTarget.style.opacity = "1"}
                >Generate from {selectedIds.length} selected →</button>
              )}
            </div>
            {githubLoading && (
              <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 16px", marginBottom: 12, background: T.card, border: `1px solid ${T.border}`, borderRadius: 8, fontFamily: "'IBM Plex Mono',monospace", fontSize: 10.5, color: T.textDim, letterSpacing: "0.04em" }}>
                <Dot color={T.purple} pulse />Fetching live GitHub commits…
              </div>
            )}
            {githubError && (
              <div style={{ padding: "12px 16px", marginBottom: 12, background: `${T.red}10`, border: `1px solid ${T.red}30`, borderRadius: 8, fontFamily: "'IBM Plex Mono',monospace", fontSize: 10.5, color: T.red, letterSpacing: "0.04em" }}>⚠ {githubError}</div>
            )}
            {!githubLoading && !githubError && allUpdates.length > 0 && (
              <div style={{ padding: "10px 14px", marginBottom: 12, background: `${T.green}0C`, border: `1px solid ${T.green}25`, borderRadius: 8, fontFamily: "'IBM Plex Mono',monospace", fontSize: 10, color: T.green, letterSpacing: "0.04em", display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {allUpdates.map(u => <UpdateRow key={u.id} update={u} selected={selectedIds.includes(u.id)} onToggle={() => toggleSelect(u.id)} />)}
            </div>
          </div>
        )}
        <footer style={{ borderTop: `1px solid ${T.border}`, marginTop: isMobile ? 40 : 56, padding: "20px 0", display: "flex", flexDirection: isMobile ? "column" : "row", justifyContent: "space-between", alignItems: isMobile ? "flex-start" : "center", gap: 12 }}>
          ...
          <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
            {SOURCES.map(s => (
              <span key={s} style={{ display: "flex", alignItems: "center", gap: 6, fontFamily: "'IBM Plex Mono',monospace", fontSize: 9.5, color: T.textDim }}>
                <SrcChip source={s} /><Dot color={T.green} pulse />
              </span>
            ))}
          </div>
        </footer>
