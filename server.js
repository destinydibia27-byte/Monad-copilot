...
app.get("/drafts", requireAuth, async (req, res) => {
  try { res.json(await getAllDrafts(req.userId)); }
  catch (e) { res.status(500).json({ error: e.message }); }
});

app.post("/drafts", requireAuth, async (req, res) => {
  try {
...
app.patch("/drafts/:id/status", requireAuth, async (req, res) => {
  try {
    const { status } = req.body;
    if (!["pending","approved","rejected","edited"].includes(status))
      return res.status(400).json({ error: "Invalid status" });
    const draft = await updateDraftStatus(req.params.id, status, req.userId);
    if (!draft) return res.status(404).json({ error: "Draft not found" });
    res.json(draft);
  } catch (e) { res.status(500).json({ error: e.message }); }
});

app.patch("/drafts/:id/text", requireAuth, async (req, res) => {
  try {
    const { text } = req.body;
    if (!text) return res.status(400).json({ error: "text is required" });
    const draft = await updateDraftText(req.params.id, text, req.userId);
    if (!draft) return res.status(404).json({ error: "Draft not found" });
    res.json(draft);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
